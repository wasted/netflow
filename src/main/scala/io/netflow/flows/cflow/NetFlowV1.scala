...
object NetFlowV1Packet {
  ...
  def apply(sender: InetSocketAddress, buf: ByteBuf): Try[NetFlowV1Packet] = Try[NetFlowV1Packet] {
    ...
    val packet = NetFlowV1Packet(sender, buf.readableBytes)
    ...
    for (i <- 0 to packet.count) NetFlowV1(sender, buf.slice(headerSize + (i * flowSize), flowSize), packet.uptime).foreach(flows += _)
    packet.flows = flows.toVector
    packet
  }
}

case class NetFlowV1Packet(sender: InetSocketAddress, length: Int) extends FlowPacket {
