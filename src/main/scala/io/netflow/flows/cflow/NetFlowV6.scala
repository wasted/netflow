...
sealed class NetFlowV6 extends CassandraTable[NetFlowV6, NetFlowV6Record] {
   ...
   */
  def apply(sender: InetSocketAddress, buf: ByteBuf, fpId: UUID, uptime: Long, ts: DateTime, si: Int): Option[NetFlowV6Record] =
    Try[NetFlowV6Record] {
      ...
      NetFlowV6Record(UUIDs.timeBased(), sender, buf.readableBytes(), uptime, ts,
