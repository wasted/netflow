...
object NetFlowV6Packet {
  ...
  def apply(sender: InetSocketAddress, buf: ByteBuf): Try[NetFlowV6Packet] = Try[NetFlowV6Packet] {
    ...
    NetFlowV6Packet(id, sender, buf.readableBytes, uptime, timestamp, flows, flowSequence, engineType, engineId, samplingInterval, samplingMode)
  }
}
...
sealed class NetFlowV6 extends CassandraTable[NetFlowV6, NetFlowV6Record] {
   ...
   */
  def apply(sender: InetSocketAddress, buf: ByteBuf, fpId: UUID, uptime: Long, timestamp: DateTime, samplingInterval: Int): Option[NetFlowV6Record] = Try[NetFlowV6Record] {
