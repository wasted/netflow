...
sealed class NetFlowV7 extends CassandraTable[NetFlowV7, NetFlowV7Record] {
   ...
   */
  def apply(sender: InetSocketAddress, buf: ByteBuf, fpId: UUID, uptime: Long, timestamp: DateTime): Option[NetFlowV7Record] =
    Try[NetFlowV7Record] {
      NetFlowV7Record(UUIDs.timeBased(), sender, buf.readableBytes(), uptime, timestamp,
  ...
  def fromRow(row: Row): NetFlowV7Record = NetFlowV7Record(id(row), new InetSocketAddress(sender(row), senderPort(row)),
...
case class NetFlowV7Record(id: UUID, sender: InetSocketAddress, length: Int, uptime: Long, timestamp: DateTime,
