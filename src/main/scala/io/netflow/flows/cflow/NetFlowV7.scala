...
sealed class NetFlowV7 extends CassandraTable[NetFlowV7, NetFlowV7Record] {
   ...
   */
  def apply(sender: InetSocketAddress, buf: ByteBuf, fpId: UUID, uptime: Long, timestamp: DateTime): Option[NetFlowV7Record] = Try[NetFlowV7Record] {
  ...
  def fromRow(row: Row): NetFlowV7Record = NetFlowV7Record(new InetSocketAddress(sender(row), senderPort(row)),
    ...
    srcMask(row), dstMask(row), flags(row), routerAddress(row), packet(row))
}
