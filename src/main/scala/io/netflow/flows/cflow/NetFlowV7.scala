...
sealed class NetFlowV7 extends CassandraTable[NetFlowV7, NetFlowV7Record] {
   ...
   */
  def apply(sender: InetSocketAddress, buf: ByteBuf, uptime: Long, timestamp: DateTime): Option[NetFlowV7Record] = Try[NetFlowV7Record] {
    NetFlowV7Record(sender, buf.readableBytes(), uptime, timestamp,
  ...
  def fromRow(row: Row): NetFlowV7Record = NetFlowV7Record(new InetSocketAddress(sender(row), senderPort(row)),
    ...
    srcMask(row), dstMask(row)) //, flags(row), routerAddress(row))
}

object NetFlowV7 extends NetFlowV7
...
case class NetFlowV7Record(sender: InetSocketAddress, length: Int, uptime: Long, timestamp: DateTime,
                           ...
                           snmpInput: Int, snmpOutput: Int, srcMask: Int, dstMask: Int) //, flags: Int, routerAddress: InetAddress // FIXME in scala 2.11
  extends NetFlowData[NetFlowV7Record] {
  def version = "NetFlowV7"

  override lazy val jsonExtra = ("srcMask" -> srcMask) ~ ("dstMask" -> dstMask) ~
    ("snmp" -> ("input" -> snmpInput) ~ ("output" -> snmpOutput)) //~ ("flags" -> flags) ~ ("routerAddress" -> routerAddress)
}
