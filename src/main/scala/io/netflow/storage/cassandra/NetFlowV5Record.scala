...
private[netflow] object NetFlowV5Packet extends FlowPacketMeta[NetFlowV5Packet] {
  def persist(fp: NetFlowV5Packet): Unit = fp.flows.foldLeft((0, new BatchStatement())) {
    case ((count, b), row) =>
      val statement = NetFlowV5Record.insert
...
private[netflow] sealed class NetFlowV5Record extends CassandraTable[NetFlowV5Record, NetFlowV5] {
  ...
  def fromRow(row: Row): NetFlowV5 = NetFlowV5(id(row), new InetSocketAddress(sender(row), senderPort(row)),
...
private[netflow] object NetFlowV5Record extends NetFlowV5Record