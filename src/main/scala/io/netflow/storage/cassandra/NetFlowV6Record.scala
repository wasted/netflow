...
private[netflow] object NetFlowV6Packet extends FlowPacketMeta[NetFlowV6Packet] {
  def persist(fp: NetFlowV6Packet): Unit = fp.flows.foldLeft((0, new BatchStatement())) {
    case ((count, b), row) =>
      val statement = NetFlowV6Record.insert
...
private[netflow] sealed class NetFlowV6Record extends CassandraTable[NetFlowV6Record, NetFlowV6] {
  ...
  def fromRow(row: Row): NetFlowV6 = NetFlowV6(id(row), new InetSocketAddress(sender(row), senderPort(row)),
...
private[netflow] object NetFlowV6Record extends NetFlowV6Record